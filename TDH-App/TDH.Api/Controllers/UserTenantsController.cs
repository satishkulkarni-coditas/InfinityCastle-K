...
public class UserTenantsController : ControllerBase
{
    ...
    {
        // Validate role
        var validRoles = new[] { "Admin", "EntityAdmin", "User" };
        if (!validRoles.Contains(request.Role))
        {
            return BadRequest(new { message = "Invalid role. Must be Admin, EntityAdmin, or User" });
        }
        ...
        // If EntityAdmin role is assigned, also add it as an Identity role if not already present
        if (request.Role == "EntityAdmin")
        {
            var user = await _userManager.FindByIdAsync(request.UserId);
            if (user != null)
            {
                var userRoles = await _userManager.GetRolesAsync(user);
                if (!userRoles.Contains("EntityAdmin"))
                {
