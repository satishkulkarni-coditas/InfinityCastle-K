# Quick Start Guide - Compliance Application
...
The Angular app will start on `http://localhost:4202`

### 3. Login

Open your browser and navigate to `http://localhost:4202`
...
### 3. Create a User
- Navigate to `/users` (AppAdmin only)
...
- Choose a role: AppAdmin, GroupAdmin, or User
- Click "Assign User"
...
2. **Or update connection string** in `Compliance.Api/appsettings.json`:
   ```json
   "ConnectionStrings": {
     "DefaultConnection": "Server=localhost;Database=Compliance_Db;Trusted_Connection=True;"
   }
   ```
...
- Ensure the API is running on `http://localhost:5002`
- Check `Program.cs` has CORS configured for `http://localhost:4202`
...
- `GET /api/users` - Get all users (AppAdmin)
- `GET /api/tenants` - Get all tenants
- `POST /api/tenants` - Create tenant (AppAdmin)
- `POST /api/usertenants/assign` - Assign user to tenant (AppAdmin)
